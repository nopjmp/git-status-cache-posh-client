...
# {
#     $release = Invoke-WebRequest -Uri "https://api.github.com/repos/cmarcusreid/git-status-cache/releases/latest" -UseBasicParsing | ConvertFrom-Json
...
#         $allReleases = Invoke-WebRequest -Uri "https://api.github.com/repos/cmarcusreid/git-status-cache/releases" -UseBasicParsing | ConvertFrom-Json | Sort-Object -Descending -Property "published_at"
...
function Update-GitStatusCache {
    ...
    Invoke-WebRequest -Uri $executableUrl -UseBasicParsing -OutFile "$executablePath"
}
...
function Start-GitStatusCache {
    ...
    if ($null -eq $process) {
        ...
        if (-not (Test-Path $executablePath)) {
            Throw [System.InvalidOperationException] "GitStatusCache.exe was not found. Please install the service."
            return $false
        }
